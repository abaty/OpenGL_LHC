...
Extrusion3D::Extrusion3D(myPolygon poly1, myPolygon poly2, unsigned int nInterpolations, int axis, float x, float y, 
		float z, float Lx, float Ly, float Lz)
	:specialFaceAxis(axis)
{
	if (poly1.getNSides() != poly2.getNSides()) {
		std::cout << "WARNING, Extrusion3D REQUIRES 2 POLYGONS HAVING SAME NUMBER OF POINTS" << std::endl;
	}

	setupBuffers(poly1, poly2, nInterpolations);
...
void Extrusion3D::setupBuffers(myPolygon poly1, myPolygon poly2, unsigned int nInterpolations) {
	...
	std::vector< myPolygon > tempPoly = polyBuilder.tube2D(poly1, 0.5, poly2, -0.5, nInterpolations, false);
...
bool Extrusion3D::insideBounds(glm::vec3 v) {
